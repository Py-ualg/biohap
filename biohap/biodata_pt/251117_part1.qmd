...
There are many parallel technologies to employ in each step. The pipeline here heavily uses `python` to enerate the graph and `fuseki` for exposing the SPARQL endpoint. Here only for local testing.
...
Expose your triples as a SPARQL end-point accessible over HTTP. Fuseki provides REST-style interaction with your RDF data. It is a java ndpoint, therefore, you might need to install `java` first if `java -version` shows nothing.
...
Now opening your localhost, http://localhost:3030/, you should see ![fuseki server](figs/fuseki_screenshot.png)

##### Upload data and generate a graph

In `fuseki`, go `new dataset` -> give it a name `emobon` -> `add data` -> select the `.ttl` files and `upload all`
