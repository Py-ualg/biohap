...
author: David Paleček ([dpalecek@ualg.pt](mailto:dpalecek@ualg.pt)), Andrzej Tkacz ([atkacz@ualg.pt](mailto:atkacz@ualg.pt))
editor: source
format:
  html:
    theme: cosmo
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  buttons.forEach(button => {
    button.addEventListener('click', function() {
      // 1. Identify the tabset and navigation links
      const tabPane = this.closest('.tab-pane');
      const tabContent = tabPane.parentElement;
      const tabsetContainer = tabContent.parentElement;
      const activeLink = tabContent.previousElementSibling.querySelector('.nav-link.active');
      const nextTabLink = activeLink.parentElement.nextElementSibling?.querySelector('.nav-link');

      if (nextTabLink) {
        // ACTION: Switch to the next tab
        nextTabLink.click();

        // ACTION: Scroll back to the top of the tabset
        // We add a small offset (e.g., 20px) so it's not jammed against the top
        const offset = 20; 
        const bodyRect = document.body.getBoundingClientRect().top;
        const elementRect = tabsetContainer.getBoundingClientRect().top;
        ...
        // ACTION: If no more tabs, go to next section
        const nextSection = tabsetContainer.nextElementSibling;
        if (nextSection) {
